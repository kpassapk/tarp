...
open( PH, "perl bin/tamasteralloc 2>&1 1>$null |" )
    or die "Error loading tamasteralloc: $!";

like( <PH>, qr/Incorrect number of arguments/, "incorrect number of args" );

open( PH, "perl bin/tamasteralloc --gen-tas foo 2>&1 1>$null |" )
    or die "Error loading tamasteralloc!";
...
open(PH, "perl bin/tamasteralloc --help 2>$null |")
    or die "Error loading tamasteralloc!";
...
open(PH, "perl bin/tamasteralloc --version 2>$null |")
    or die "Error loading tamasteralloc!";
    
my $version_msg = <PH>;

ok( $version_msg =~ /tamasteralloc v(\d\.\d{2,})/, "verison msg format" )
    or diag "Got version message: $version_msg";

SKIP: {
    skip( "no version number", 1 ) unless $1;
    ok( $1 >= $Tarp::VERSION, "version is Tarp version" )
        or diag "Need to set \$VERSION in MasterAlloc.pm to $Tarp::VERSION";
}
